...
- 支持本地黄历、常见万年历、传统通胜、黄历原文四种来源口径，并可手动刷新检测。
...
- `本地黄历（6tail）`：使用内置离线库，适合纯静态部署。
- `常见万年历（宜忌优先）`：模拟许多万年历产品更重视每日宜忌的展示习惯。
- `传统通胜（保守）`：模拟更保守的筛选方式，黑道、冲煞、建除不利会更强降级。
- `黄历原文（轻评分）`：便于和外部黄历页面人工对照。

纯静态页面不直接抓取市面黄历网页，主要是为了避免跨域、授权、版权和页面结构变化问题；如果后续提供稳定 API，可在当前来源适配层继续接入。
